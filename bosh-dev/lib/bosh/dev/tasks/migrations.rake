namespace :migrations do
  ...
  namespace :schema do
    ...
    task :dump do
      ...
      director_config = {
        'db' => {
          'database' => "sqlite:///tmp/something.sqlite", #"#{path.path}.sqlite",
          'adapter' => 'sqlite',
          'host' => '127.0.0.1'
        }
      }
      ...
      db = Bosh::Director::Config.configure_db(director_config['db'])
      db.extension :bosh_schema_caching
      db.dump_schema_cache(File.expand_path('../../../../../../bosh-director/db/schema.dump', __FILE__))
    end
  end
end
