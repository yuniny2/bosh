  ...
  module Director
    describe VmCreator do
      subject { VmCreator.new(
          Config.cloud, logger, vm_deleter, disk_manager, job_renderer, agent_broadcaster, dns_manager
      ) }

      let(:disk_manager) { DiskManager.new(Config.cloud, logger) }
      let(:vm_deleter) { VmDeleter.new(Config.cloud, logger, dns_manager, false, false) }
