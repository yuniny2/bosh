...
Cannot specify 'properties' without 'instances' for link 'link_name' in job 'foo' in instance group 'job_name'./)
  ...
  describe '#bind_template_scoped_properties' do
    ...
    it 'should drop user provided properties (from deployment manifest, or potentially CLI) not specified in the release job spec properties' do
    ...
    it 'should include properties that are in the release job spec but not provided by a user ( deployment manifest or CLI)' do
    ...
    it 'should not override user provided properties ( from deployment manifest or CLI ) with release job spec defaults' do
      template.bind_template_scoped_properties('instance_group_name')
