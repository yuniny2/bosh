...
module Bosh::Director
  describe ProblemScanner::VmScanStage do
    ...
    def create_vm(i, state: 'started', lifecycle: 'service', ignore: false, vm_cid: "vm-cid-#{i}")
      instance = Models::Instance.make(vm_cid: vm_cid, agent_id: "agent-#{i}", deployment: deployment, job: "job-#{i}", index: i, state: state, ignore: ignore)
      instance_groups << double(name: "job-#{i}", lifecycle: lifecycle)
      instance
    end
