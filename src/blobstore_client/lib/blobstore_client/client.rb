  ...
  module Blobstore
    class Client
      ...
      def self.safe_create(provider, options = {})
        ...
        if options['verify-multidigest'].nil? || options['verify-multidigest']['path'].nil?
          raise BlobstoreError,
                'Multiple Digest binary not provided'
        end
        sha1_client    = Sha1VerifiableBlobstoreClient.new(wrapped_client, options['verify-multidigest']['path'])
