    ...
    class BaseLinkLookup
      ...
      def update_addresses!(link_spec)
        ...
        unless @use_dns_entries
          unless network_name
            raise Bosh::Director::LinkLookupError,
              "Unable to retrieve default network '#{network_name}' from provider – " +
                "preferred network was '#{@preferred_network_name}', default network was '#{link_spec['default_network']}'. " +
                'Please redeploy provider deployment.'
          end
        end
        ...
        if network_name
          link_spec['instances'].each do |instance|
            ...
            unless addresses
              raise Bosh::Director::LinkLookupError, 'Unable to retrieve network addresses. Please redeploy provider deployment'
            end

            unless addresses.key?(network_name)
              raise Bosh::Director::LinkLookupError, "Invalid network name: #{network_name}"
            end
