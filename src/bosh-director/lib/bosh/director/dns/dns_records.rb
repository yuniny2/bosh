module Bosh::Director
  class DnsRecords
    ...
    def initialize(version, include_index_records, dns_query_encoder)
      @version = version
      @record_keys = ['id', 'global_index', 'instance_group', 'group_ids', 'az', 'az_id', 'network', 'network_id', 'deployment', 'ip', 'domain', 'agent_id', 'instance_index']
      @record_infos = []
      @records = []
    ...
    def add_record(instance_id, global_index, index, instance_group_name, az_name, network_name, network_id, deployment_name, ip, domain, agent_id)
