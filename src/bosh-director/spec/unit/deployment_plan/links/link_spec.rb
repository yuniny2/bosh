  ...
  module DeploymentPlan
    describe Link do
      ...
      before do
        ...
        allow(source_instance_group).to receive(:default_network_name).and_return(network_name)
      ...
      context '#spec' do
        context 'when there is an instance plan' do
          before do
            ...
            allow(needed_instance_plan).to receive(:network_address).and_return('10.0.0.1')
            expect(needed_instance_plan).to receive(:network_addresses).with(true).and_return({'smurf_network' => 'network-address-1', 'private_network_name' => 'network-address-2'})
            expect(needed_instance_plan).to receive(:network_addresses).with(false).and_return({'smurf_network' => '10.0.0.1', 'private_network_name' => '10.0.0.2'})
              ...
              'instances' => [
                {
                  ...
                  'az' => 'my_az',
                  'address' => '10.0.0.1',
                  'addresses' => {'smurf_network' => 'network-address-1', 'private_network_name' => 'network-address-2'},
                  'ip_addresses' => {'smurf_network' => '10.0.0.1', 'private_network_name' => '10.0.0.2'}
                }
              ]
            })
          end
        ...
        context 'without network name' do
          ...
          before {
            allow(source_instance_group).to receive(:needed_instance_plans).and_return([])
            allow(source_instance_group).to receive(:default_network_name).and_return('smurf_network')
          }
