...
module Bosh::Director
  describe Errand::ErrandProvider do
    ...
    describe '#get' do
      let(:deployment_planner) { instance_double(DeploymentPlan::Planner, template_blob_cache: template_blob_cache, ip_provider: ip_provider) }
      ...
      context 'when running an errand by instance group name' do
        ...
        context 'when there is a lifecycle: errand instance group with that name' do
          ...
          it 'returns an errand object that will run on the first instance in that instance group' do
            ...
            expect(JobRenderer).to receive(:render_job_instances_with_cache).with(needed_instance_plans, template_blob_cache, logger)
