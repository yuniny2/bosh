    ...
    context 'config server' do
      context 'when turned on' do
        ...
        it 'configures the cpi correctly' do
          ...
          expect(parsed_yaml['config_server']['uaa_url']).to eq('fake-uaa-url')
          expect(parsed_yaml['config_server']['uaa_client_id']).to eq('fake-client-id')
          expect(parsed_yaml['config_server']['uaa_client_secret']).to eq('fake-client-secret')
          expect(parsed_yaml['config_server']['uaa_ca_cert_path']).to eq('/var/vcap/jobs/director/config/uaa_server_ca.cert')
        end
      end
